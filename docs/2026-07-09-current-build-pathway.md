...
| Item | Status | Notes |
|---|---|---|
| Browser/PWA app | live; application build current | Production URL: `https://ai-task-router.pages.dev/`; latest deployment `https://eb485f5c.ai-task-router.pages.dev` from source `e3671b2`, deployed 2026-07-27. It carries the Stage 2/3 repair and the followed-choice clarification; the only post-deploy `main` changes are the hosted-test readiness fix and release records. |
...
| Active chunk | Followed-choice clarification: task complete and live | Historical plans were saved as Deferred before new saves changed to Accepted, while the counter only includes Accepted or Edited outcomes. The panel now states that contract, explains why saved-but-uncounted choices leave it at zero, and provides a Review saved choices action into Past Choices. No outcome was silently migrated because the app cannot infer whether a route was actually used. Full local and hosted release gates pass, and production serves the exact verified build. |
...
**The production application build is current at source `e3671b2`, deployed 2026-07-27; the only post-deploy `main` changes are the hosted-test readiness fix and release records.** The deployment carries the Stage 2/3 repair and the followed-choice clarification. Canonical production serves the exact 661,862-byte local asset; `Accepted or edited choices`, `Review saved choices`, and the saved-but-uncounted explanation are present; the old exact `Followed choices` header is absent; and hosted E2E passes 7/7. The repair is deliberately narrow: no provider connection, execution behavior, analytics, or silent outcome migration was added.
