...
**Production is current with `main`.** The owner authorized two deploys on 2026-07-26: `69b31a2` at 10:21:53-06:00 and `a34d839` at 11:10:28-06:00. R7's Help screen, R6's catalog staleness notice — silent until the catalog passes 90 days on 2026-10-03 — the money formatter that shows real dollars and cents, and the three teaching-audit fixes are all in front of users now.

**A teaching audit on 2026-07-26 found six gaps; five are fixed and one is open.** The question asked was narrower than the 2026-07-25 functional audit: not "does the app work" but "does it teach why right-sizing matters". Fixed and live as of `a34d839`: every energy figure a user decides on now carries an everyday restatement, because watt-hours alone were honest, sourced, and unreadable; the lean/balanced/premium split of followed routes is now rendered, having been computed and discarded since it was written; and heaviest-sibling comparisons now carry the multiple instead of leaving the reader to divide. Still open, in rough priority order:

1. ~~**The lesson arrives after the decision.**~~ Fixed 2026-07-26T11:20:59-06:00. Start Here carries three worked examples before any task is described.
2. ~~**Past Choices teaches nothing.**~~ Fixed 2026-07-26T11:20:59-06:00. Rows carry metered cost, energy, and a comparison against the heaviest route offered; the log carries choice-pattern totals.
3. **Two of the four impact metrics are hedged into invisibility.** "Textbook example, not your usage" and "Illustrative only... Nobody is claiming you ran these" are honest but occupy half the panel's visual weight while teaching less than the two task-specific figures beside them. The right-sizing example is the strongest teaching content in the app — it nets out induced extra runs, so it admits that going too small costs more — and its label invites the reader to skip it. Partly addressed by finding 1's fix, which promotes that same example to Start Here and states the too-small-costs-more point in the body rather than in a hedge; what remains is the Best Options panel itself, where the wording and visual weight are unchanged. Whoever takes it should treat the hedges as load-bearing and rebalance the weight rather than delete them.
