...
Reference only as of 2026-07-06T15:24:36.2422654-06:00: the owner wants Old Skool AI to be the primary public product/download hub, with `guidedailabs.com` and `guidedaijourney.com` linking there. The owner also wants Windows, Mac, and Linux downloads as soon as possible. D10 adds a manual technical-preview artifact lane for Windows/macOS/Linux builds, but public ordinary-user desktop downloads remain held until signing/notarization, checksums, install/launch/uninstall smoke, local discovery smoke, support/withdrawal copy, and owner launch approval pass. D11 adds the public launch master plan in [docs/2026-07-04-public-launch-master-plan.md](2026-07-04-public-launch-master-plan.md), making the Old Skool AI hub, hosted web/PWA, desktop trust gates, support/withdrawal needs, and go/no-go board the controlled release map. D12 adds the [Old Skool AI Hub Handoff Package](2026-07-04-old-skool-ai-hub-handoff.md) with Linux-side page copy, cross-site links, held desktop-download copy, publish checklist, and rollback notes. D13 adds the [Cloudflare Production Launch Smoke](2026-07-05-cloudflare-production-launch-smoke.md), selecting and smoking `https://ai-task-router.pages.dev/` as the first canonical online app URL. D14 adds the [Public Hub And Cross-Site Link Smoke](2026-07-05-public-hub-and-cross-site-link-smoke.md), publishing the Old Skool AI hub, public security route, and Guided AI Labs / Guided AI Journey links. D15 adds the [AI Task Router Impact Estimator Methodology](2026-07-05-impact-estimator-methodology.md), a calculation backbone for 100k-token pricing, right-sizing savings, and scenario-based environmental estimates. D16 adds the [Public Impact Insight Cloudflare Update](2026-07-05-public-impact-insight-cloudflare-update.md), wiring the impact story into the public Best Options UI and redeploying the Cloudflare Pages production app at `https://ai-task-router.pages.dev/`. D17 adds the [Desktop Download Readiness Gate](2026-07-05-desktop-download-readiness-gate.md), making technical-preview artifact/checksum checks repeatable and making the public desktop gate fail until trust evidence exists. D18 adds the [Public Stage Guidance Cloudflare Update](2026-07-05-public-stage-guidance-cloudflare-update.md), wiring compact suggested stages and recommended help into Best Options, saved Decision Cards, and Markdown exports, then redeploying the production app. D19 adds the [Public PDF Report Cloudflare Update](2026-07-05-public-pdf-report-cloudflare-update.md), wiring a browser print/save-PDF report path into saved Decision Cards and redeploying the production app. D20 adds the [Desktop Public Distribution Decision](2026-07-06-desktop-public-distribution-decision.md), recommending Windows Store/MSIX first, adding a public desktop release evidence template, and tightening `npm run desktop:gate:public` so it only passes with real platform trust evidence. D21 adds the [Windows MSIX Proof](2026-07-06-windows-msix-proof.md), creating a repeatable self-signed MSIX proof package path and workflow support while keeping public desktop downloads held. D22 adds the [Windows Store Trust Prep](2026-07-06-windows-store-trust-prep.md), adding a Partner Center identity template and guarded manifest-prep command so the MSIX manifest can be updated after the owner reserves the app. D9's preview alias remains historical smoke evidence only and must not be used in public links.

## Future Desktop Trust Note

Reference only as of 2026-07-06T15:24:36.2422654-06:00: the project should keep two distribution options open: a hosted/PWA web app for low-friction use and a signed desktop app for permissioned local machine discovery. The detailed planning baseline is [docs/2026-07-04-desktop-trust-distribution-plan.md](2026-07-04-desktop-trust-distribution-plan.md), the public release execution map is [docs/2026-07-04-public-launch-master-plan.md](2026-07-04-public-launch-master-plan.md), and the public website handoff is [docs/2026-07-04-old-skool-ai-hub-handoff.md](2026-07-04-old-skool-ai-hub-handoff.md). Desktop Chunk D0 is confirmed for planning, Desktop Chunk D1 selected Tauri for the first shell spike in [ADR-0001](decisions/adr-0001-desktop-wrapper.md), Desktop Chunk D2 has the shell scaffold and Windows prerequisites, Desktop Chunk D3 defined the trust-boundary contract and CSP hardening, Desktop Chunk D4 implements a desktop-only `Check this computer` local AI tool discovery flow using custom Rust commands, Desktop Chunk D5 implements the hosted/browser PWA install path, Desktop Chunk D6 adds an opt-in internal Windows NSIS packaging path plus signing requirements documentation, Desktop Chunk D7 records the release/security readiness packet in [docs/2026-07-04-release-security-readiness-packet.md](2026-07-04-release-security-readiness-packet.md), D8 records the web/PWA release-candidate security pass in [docs/2026-07-04-web-release-candidate-security-pass.md](2026-07-04-web-release-candidate-security-pass.md), D9 records the Cloudflare Pages hosted preview smoke in [docs/2026-07-04-cloudflare-pages-hosted-preview-smoke.md](2026-07-04-cloudflare-pages-hosted-preview-smoke.md), D10 records the manual desktop technical-preview artifact lane in [docs/2026-07-04-desktop-technical-preview-artifacts.md](2026-07-04-desktop-technical-preview-artifacts.md), D11 records the public launch master plan, D12 records the Old Skool AI hub handoff package, D17 records the desktop download readiness gate, D18 records the public stage-guidance update, D19 records the public PDF report update, D20 records the public desktop distribution decision, D21 records the Windows MSIX proof, and D22 records the Windows Store trust prep. D4 keeps broad Tauri plugin permissions empty, rejects path details, hides model names by default, does not run startup/background scans, and validates with build-only desktop checks because Windows Application Control still blocks dev-mode, unsigned release executable launch, and final execution of the generated release Rust test binary. D5 adds manifest/install icons/service worker/install copy only; browser/PWA local discovery remains prohibited. D6 generated an unsigned internal NSIS artifact for evidence only; D10 can generate unsigned/unnotarized technical-preview artifacts for platform verification only; D17 checks artifact/checksum hygiene; D20 recommends Windows Store/MSIX first and keeps `desktop:gate:public` blocking until a real `docs/release/desktop-public-release-evidence.json` records platform trust evidence; D21 proves self-signed local MSIX packaging only; D22 prepares the manifest for real Partner Center package identity values without creating a public release claim. Public desktop release remains held until desktop signing/trust evidence where applicable and owner launch decision pass. The terminal `npm run detect:local-models` command remains separate and explicit.
...
| Public impact insight UI | task complete, public web updated | 2026-07-05T09:34:16-06:00 | Technical Lead | Added [Public Impact Insight Cloudflare Update](2026-07-05-public-impact-insight-cloudflare-update.md), a public Best Options impact section, a Start Here skill-building cue, reviewed source snapshot module, source-link release allowlist, tests, local visual smoke, and Cloudflare production deployment from source `b4daec6`. Exact per-user savings claims, live pricing fetches, social launch, custom domains, and desktop downloads remain gated. |
| Desktop download readiness gate | task complete, public download hold | 2026-07-05T10:07:48-06:00 | Technical Lead | Added [Desktop Download Readiness Gate](2026-07-05-desktop-download-readiness-gate.md), `scripts/desktop-release-gate.mjs`, script tests, npm gate commands, and a technical-preview workflow gate step. Fresh Windows technical-preview packaging and gate validation passed; public gate failed as expected until platform trust evidence exists. |
| Public suggested-stage guidance | task complete, public web updated | 2026-07-05T10:49:02-06:00 | Technical Lead | Added [Public Stage Guidance Cloudflare Update](2026-07-05-public-stage-guidance-cloudflare-update.md), route-card `stageGuidance`, compact Best Options and Decision Card stage panels, Markdown export support, focused/full tests, local/hosted smoke, and Cloudflare production deployment from source `9d3154d`. Live pricing/model fetches, provider connections, social launch, custom domains, and desktop downloads remain gated. |
| Public PDF report | task complete, public web updated | 2026-07-05T11:12:24-06:00 | Technical Lead | Added [Public PDF Report Cloudflare Update](2026-07-05-public-pdf-report-cloudflare-update.md), shared impact panel, saved Decision Card impact context, browser `Save PDF report` action, print CSS, focused/full tests, local/hosted PDF report smoke, and Cloudflare production deployment from source `9c870ce`. Server-rendered PDFs, provider connections, social launch, custom domains, live pricing/model fetches, exact savings claims, and desktop downloads remain gated. |
...
| PDCA planning simplification | task complete | 2026-07-08T18:54:14-06:00 | Technical Lead | Simplified visible planning labels to `Plan`, `Do`, `Check`, `Act`; updated generated prompt guidance to require only those four sections; exposed mode and upgrade trigger in stage routing detail; preserved deliverable-level helper routing for prompt, build, evidence, package, review, and next-action work. Validation passed: governance preflight, focused unit tests, full unit suite, production build, Playwright MVP workflow smoke, and `git diff --check` with normal Windows line-ending notices. |
...
## Desktop Chunk D17 - Desktop Download Readiness Gate

Status: task complete, public download hold
...
Add a machine-checkable gate for generated desktop packages so technical-preview artifacts can be verified without accidentally treating them as public-release downloads.

User outcome:

The owner has a clearer path toward "Download for Windows/Mac/Linux" buttons: artifacts and checksums can be checked repeatably, and the public gate remains visibly blocked until platform trust evidence exists.
...
D17 does not add new desktop permissions, provider connections, telemetry, credentials, broad filesystem access, updater behavior, or external calls. It adds artifact/checksum checks and a public gate that is expected to fail until signing/notarization, smoke, support/withdrawal, and owner approval evidence exists.

Implementation:

- Added [Desktop Download Readiness Gate](2026-07-05-desktop-download-readiness-gate.md).
- Added `scripts/desktop-release-gate.mjs`.
- Added Node script tests for technical-preview hold behavior, public gate blocking, stale checksum detection, and artifact platform classification.
- Added `npm run desktop:gate:technical-preview`.
- Added `npm run desktop:gate:public`.
- Added the technical-preview gate to `.github/workflows/desktop-technical-preview.yml` after checksum generation and before artifact upload.

Release decision:

Hold public desktop downloads. Technical-preview artifacts may be generated for owner/developer inspection only. `npm run desktop:gate:public` should remain failing until platform trust evidence exists.

Rollback or recovery:

Remove `scripts/desktop-release-gate.mjs`, `scripts/desktop-release-gate.node-test.mjs`, the new package scripts, and the workflow gate step. No provider-side release, public download link, signing key, DNS state, or updater state is created by this chunk.

Stop condition:

Stop if the gate cannot reliably detect missing artifacts, stale checksums, or missing public trust evidence.

## Public Web Chunk D18 - Suggested Stage Guidance

Status: task complete, public web updated
...
Add compact inferred work stages beside the existing route recommendation so users can organize the task and see the recommended tool/model or human review beside each stage.

User outcome:

Users see a rough path such as framing, gathering/checking, creating, packaging when relevant, and review, without being handed a full project plan or any automated execution workflow.

Allowed files or folders:

- route-card schema and type files
- route-card generation and export code
- Best Options and Decision Card UI
- focused tests and app tests
- release, runbook, changelog, README, domain-language, and pathway docs

Non-goals:

- Full project-plan generation
- Live model lookup
- Provider entitlement or subscription verification
- Provider account connections
- Live pricing or model fetches
- Automation or execution workflows
- Social launch posts
...
D18 does not add external calls, telemetry, credentials, account connections, file access, local discovery expansion, or provider API usage. Stage guidance is generated locally from the task, selected route, and user-configured tool labels.

Implementation:

- Added [Public Stage Guidance Cloudflare Update](2026-07-05-public-stage-guidance-cloudflare-update.md).
- Added `src/domain/routing/stageGuidance.ts`.
- Added backward-compatible `stageGuidance` route-card schema support.
- Added Suggested stages UI in Best Options and saved Decision Cards.
- Added Suggested Stages to route-card Markdown export.
- Added focused tests for schema compatibility, route-card generation, research-stage model labels, app visibility, and Markdown export.
- Redeployed Cloudflare Pages production from source `9d3154d` to deployment `98a58ca6`.

Release decision:

Public web update complete at `https://ai-task-router.pages.dev/`. Keep social launch, DNS/custom-domain work, desktop downloads, provider connections, live pricing/model fetches, and exact public savings claims gated behind separate approval and evidence.

Rollback or recovery:

Roll Cloudflare Pages production back from `98a58ca6-a75a-4ff6-9cd9-ab8af31c834b` to `cd2c5112-e554-407f-a631-114fc189651c`, or deploy a corrected `dist/` artifact. Revert source commit `9d3154d` if the stage-guidance schema, copy, or UI should be removed.

Stop condition:

Stop if stage guidance crowds the routing UI, breaks saved route-card compatibility, fails web release scans, or requires live provider/model lookup.

## Public Web Chunk D19 - PDF-Ready Decision Reports
...
Add a durable report form for users who want to share or keep a saved decision without relying on screenshot capture.

User outcome:

Users can save a route, open the saved Decision Card, and choose `Save PDF report` to use the browser print/save-PDF path. The report includes the decision summary, suggested stages, impact context, warnings, and route tradeoffs.

Allowed files or folders:

- saved route-card artifact UI
- shared impact UI
- print CSS
- focused App tests
- release, runbook, changelog, README, domain-language, and pathway docs
...
- Screenshot capture
- New PDF library dependency
- Provider calls or account connections
- Live pricing/model fetches
- Exact public savings claims
- Social launch posts
- DNS/custom-domain changes
- Desktop download links
...
D19 does not add external calls, telemetry, credentials, account connections, file access, local discovery expansion, or provider API usage. The PDF-ready report is produced by the user's browser print flow from locally saved Decision Card content.

Implementation:

- Added [Public PDF Report Cloudflare Update](2026-07-05-public-pdf-report-cloudflare-update.md).
- Added `src/ui/screens/ImpactInsightPanel.tsx` as the shared impact panel.
- Added impact context to saved Decision Cards.
- Added `Save PDF report` to saved Decision Cards.
- Added print-only report header and print CSS that hides navigation/export controls while preserving report content.
- Added focused App coverage for the report action and saved-card impact context.
- Redeployed Cloudflare Pages production from source `9c870ce` to deployment `49d21829`.

Release decision:

Public web update complete at `https://ai-task-router.pages.dev/`. Keep server-rendered PDFs, social launch, DNS/custom-domain work, desktop downloads, provider connections, live pricing/model fetches, and exact public savings claims gated behind separate approval and evidence.

Rollback or recovery:

Roll Cloudflare Pages production back from `49d21829-e6de-4ef0-b2e0-170621b9d16e` to `98a58ca6-a75a-4ff6-9cd9-ab8af31c834b`, or deploy a corrected `dist/` artifact. Revert source commit `9c870ce` if the shared impact panel, saved Decision Card impact section, print button, or print CSS should be removed.

Stop condition:

Stop if the report path requires a server renderer, sends data off device, breaks Markdown export, crowds the Best Options flow, fails print smoke, or requires new external dependencies.
...
| 2026-07-05T09:34:16-06:00 | `bash scripts/governance-preflight.sh`; `npm run test -- impactEstimator`; `npm run test -- App`; `npm run test`; `npm run build`; `npm run scan:web-rc`; `npx playwright test`; local visual smoke; Wrangler production deploy; hosted asset/E2E/Chromium smoke | passed with existing build warning | D16 public impact insight validation passed: governance preflight reported 0 warnings; focused impact/App tests passed; full Vitest passed 13 files and 95 tests; production build passed with the existing Vite chunk-size warning; web RC scan passed; local Playwright passed 6 Chromium tests; desktop/mobile preview and source-details-open smoke had no horizontal overflow; Cloudflare production deployment `cd2c5112` from source `b4daec6` passed hosted asset checks, hosted Playwright, impact text smoke, 0 initial external requests, and no desktop overflow. |
| 2026-07-05T10:07:48-06:00 | `bash scripts/governance-preflight.sh`; official source review; `node --check scripts/desktop-release-gate.mjs`; `node --check scripts/desktop-release-gate.node-test.mjs`; `npm run test:scripts`; `npm run test`; `npm run build`; `npm run scan:web-rc`; `npm audit --audit-level=moderate`; `npx --yes yaml-lint .github/workflows/desktop-technical-preview.yml`; `npm run desktop:package:windows:technical-preview`; `npm run desktop:artifacts`; `npm run desktop:checksums`; `npm run desktop:gate:technical-preview`; `npm run desktop:gate:public`; `Get-AuthenticodeSignature`; `git diff --check` | passed with expected public gate failure and existing build warning | D17 validation passed: governance preflight 0 warnings; official Tauri/Microsoft/Apple signing and pipeline sources reviewed; script syntax checks and 8 Node script tests passed; full Vitest passed 13 files and 95 tests; production build passed with existing Vite chunk-size warning; web RC scan passed; audit found 0 vulnerabilities; workflow YAML parsed; Windows technical-preview NSIS package built with SHA-256 `F7086F7F4D87379111F81FC9F839C88C566B46C3F1E931280DBE1E18E4CD80B4`; `desktop:gate:technical-preview` passed artifact hygiene and printed the public-download hold; `desktop:gate:public` failed as expected for missing trust evidence; installer and executable reported `NotSigned`; whitespace check reported only normal Windows LF-to-CRLF notices. |
| 2026-07-05T10:49:02-06:00 | `bash scripts/governance-preflight.sh`; `npx tsc --noEmit`; `npm run test -- routeCardGenerator domainSchemas exportImport App`; `npm run test`; `npm run build`; `npm run scan:web-rc`; `npm audit --audit-level=moderate`; `npx playwright test`; local visual smoke; Wrangler production deploy; hosted asset/E2E/Chromium smoke; `git diff --check` | passed with existing build warning | D18 validation passed: governance preflight 0 warnings; focused tests passed 4 files and 35 tests; full Vitest passed 13 files and 96 tests; production build passed with existing Vite chunk-size warning; web RC scan passed; audit found 0 vulnerabilities; local Playwright passed 6 Chromium tests; local desktop/mobile visual smoke rendered Suggested stages without horizontal overflow; Cloudflare production deployment `98a58ca6` from source `9d3154d` passed hosted asset checks, hosted Playwright, stage-guidance smoke with 4 stage cards, 0 initial external requests, and no desktop overflow; whitespace check reported only normal Windows LF-to-CRLF notices. |
| 2026-07-05T11:12:24-06:00 | `bash scripts/governance-preflight.sh`; `npx tsc --noEmit`; `npm run test -- App`; `npm run test`; `npm run build`; `npm run scan:web-rc`; `npm audit --audit-level=moderate`; `npx playwright test`; local PDF-report smoke; Wrangler production deploy; hosted asset/E2E/PDF-report smoke; `git diff --check` | passed with existing build warning | D19 validation passed: governance preflight 0 warnings; focused App suite passed 14 tests; full Vitest passed 13 files and 96 tests; production build passed with existing Vite chunk-size warning; web RC scan passed; audit found 0 vulnerabilities; local Playwright passed 6 Chromium tests; local and hosted PDF-report smokes rendered `Save PDF report`, impact context, and print CSS with no desktop/mobile overflow; hosted smoke observed 0 external requests; Cloudflare production deployment `49d21829` from source `9c870ce` passed hosted asset checks and hosted Playwright; whitespace check reported only normal Windows LF-to-CRLF notices. |
| 2026-07-06T13:55:02-06:00 | `bash scripts/governance-preflight.sh`; official source refresh; `node --check scripts/desktop-release-gate.mjs`; `node --check scripts/desktop-release-gate.node-test.mjs`; `npm run test:scripts`; `npm run test`; `npm run build`; `npm audit --audit-level=moderate`; `npm run desktop:gate:technical-preview`; `npm run desktop:gate:public`; `git diff --check` | passed with expected public gate failure and existing build warning | D20 validation passed: governance preflight reported 0 warnings; Microsoft, Apple, and Tauri distribution/signing docs were reviewed; script syntax checks passed; Node script suite passed 10 tests; full Vitest passed 13 files and 96 tests; production build passed with the existing Vite chunk-size warning; audit found 0 vulnerabilities; technical-preview gate passed artifact hygiene and printed the public-download hold; public gate failed as expected because no real `docs/release/desktop-public-release-evidence.json` exists and Windows trust evidence remains incomplete; whitespace check reported only normal Windows LF-to-CRLF notices. |
| 2026-07-06T14:08:49-06:00 | `bash scripts/governance-preflight.sh`; official source refresh; `winget install Microsoft.WinAppCLI`; `winapp --version`; `winapp manifest generate`; `node --check scripts/package-windows-msix-proof.mjs`; `node --check scripts/package-windows-msix-proof.node-test.mjs`; `npm run test:scripts`; `npm run desktop:package:windows:msix-proof`; `npm run desktop:package:windows:msix-proof -- --skip-build`; `npm run desktop:artifacts`; `npm run desktop:gate:technical-preview`; `npm run desktop:gate:public`; `Get-AuthenticodeSignature`; package content listing | passed with expected public gate failure and existing build warning | D21 validation passed: governance preflight reported 0 warnings; Microsoft WinApp/Tauri and Tauri Windows docs were reviewed; WinApp CLI 0.4.0 installed locally; manifest/assets generated; Node script suite passed 13 tests; local MSIX proof package generated with SHA-256 `6668983AE21279E918F1DE6A34E37369E0A6B0BF3E78D0827CF0C0A6AD80EDCD`; technical-preview gate passed for MSIX plus NSIS artifacts; public gate failed as expected; package contains manifest/exe/block map/signature; signature uses local `CN=Guided AI Labs Ltd` development certificate with untrusted root. |
| 2026-07-06T14:25:40-06:00 | `npm run test`; `npm run build`; `npm audit --audit-level=moderate`; `npx --yes yaml-lint .github/workflows/desktop-technical-preview.yml`; `bash scripts/governance-preflight.sh`; `git diff --check` | passed | D21 close-out validation passed: full Vitest passed 13 files and 96 tests; production build passed; audit found 0 vulnerabilities; workflow YAML lint passed; governance preflight reported 0 warnings; whitespace check reported only normal Windows LF-to-CRLF notices. |
...
## Completed Chunk - PDCA Planning Simplification
...
- Updated `stageGuidance.ts` so visible stage method labels are only `Plan`, `Do`, `Check`, and `Act`.
- Updated route-step and prompt-package instructions so generated planning prompts ask for four sections only: `Plan`, `Do`, `Check`, `Act`.
- Updated `StageGuidancePanel.tsx` so expanded routing detail shows recommended help, mode when available, estimate, and upgrade trigger.
- Updated route-card, prompt-package, export/import, schema, and local-store tests so saved cards and Markdown exports preserve the simplified language.

Validation evidence:

- `bash scripts/governance-preflight.sh` passed with 0 warnings.
- `npm run test -- routeCardGenerator promptPackageGenerator exportImport domainSchemas storageLocalStore` passed: 5 files, 44 tests.
- `npm run test` passed: 14 files, 116 tests.
- `npm run build` passed with the existing Vite chunk-size warning.
- `npx playwright test src/tests/e2e/mvp-workflows.spec.ts --project=chromium` passed: 6 tests.
- `git diff --check` passed with normal Windows LF-to-CRLF notices.
