...
The browser/PWA app is live at `https://ai-task-router.pages.dev/`, the Old Skool AI public hub is live at `https://oldskoolai.com/ai-task-router/`, The desktop track was abandoned on 2026-07-26 - the Tauri shell, local discovery, packaging scripts, and the "Desktop app - Coming Soon!" card are gone, and [ADR-0002](docs/decisions/adr-0002-abandon-desktop-track.md) records why. This is a web app; there is nothing to install beyond the browser's own install option. Latest production deployment: `https://9d00dce4.ai-task-router.pages.dev` from source `a34d839`, deployed 2026-07-26T11:10:28-06:00 on owner authorization. That deploy carried the three teaching-audit fixes — readable energy figures, the followed-route lean/balanced/premium split, and comparison multiples; the 2026-07-26T10:21:53-06:00 deploy before it carried R6, R7, R8, R9, the bundle budget gate, the desktop-track removal, and the leanness pass, so the Help tab shows a real screen and route costs read in real dollars and cents. **`main` has since moved ahead of production again:** teaching-audit findings 1 and 2 — the pre-task worked examples on Start Here, and cost, energy, and choice-pattern totals on Past Choices — are committed but not deployed, and a deploy needs owner authorization. The Cloudflare deploy blocker is resolved — the token was never the problem, only the network location it was used from, and the recovery path in `docs/2026-07-09-cloudflare-deploy-turnover.md` has now worked unchanged from the home network three times. Use that note as the deploy runbook for the next release. Suggested-stage guidance currently lives in `src/domain/routing/stageGuidance.ts` and `src/ui/screens/StageGuidancePanel.tsx`. The latest routing-feedback fix makes chart points expose hover/focus values, makes routing detail show concrete decision/reason/check/upgrade fields, makes complex build-planning tasks split the build stage into concrete build items instead of collapsing into prompting only, and keeps ordinary planning wording like "build an itinerary" out of app-build routing unless the task asks for an app, tool, code, automation, workflow, or build slice. The PWA service worker uses the `ai-task-router-pwa-v2` cache and network-first same-origin asset fetching so returning browsers do not prefer stale app bundles after a release.
