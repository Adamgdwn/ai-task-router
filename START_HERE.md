...
**Resume point: [docs/2026-07-26-session-turnover.md](docs/2026-07-26-session-turnover.md).** Written at the close of the 2026-07-26 session. Read it first, then this file, then the Next Handoff in the active pathway below.

Default live build route: [docs/2026-07-09-current-build-pathway.md](docs/2026-07-09-current-build-pathway.md).

Current coder queue: [docs/2026-07-25-audit-remediation-plan.md](docs/2026-07-25-audit-remediation-plan.md), chunks R0 through R12 from the 2026-07-25 functional audit. Each chunk names its own context load. All thirteen chunks, R0 through R12, are complete as of 2026-07-25T16:58:01-06:00, and both owner decisions are recorded in that file. There is no next coder chunk in that queue, and the production deploy it was waiting on landed 2026-07-26T10:21:53-06:00. A separate teaching audit on 2026-07-26 then asked whether the app teaches *why* right-sizing matters and found six gaps; all six are fixed and live as of `e58f81b`. The 2026-07-27 Stage 2 reasoning and Stage 3 project-plan repair is integration-complete and will be on `main` after closeout, but is not deployed without separate release authorization. The remaining open items are in `CARRY_FORWARD.md`. Note that R5 turned on `noUnusedLocals` and `noUnusedParameters`, so unused code now fails `npm run build`.
...
The browser/PWA app is live at `https://ai-task-router.pages.dev/`, the Old Skool AI public hub is live at `https://oldskoolai.com/ai-task-router/`, The desktop track was abandoned on 2026-07-26 - the Tauri shell, local discovery, packaging scripts, and the "Desktop app - Coming Soon!" card are gone, and [ADR-0002](docs/decisions/adr-0002-abandon-desktop-track.md) records why. This is a web app; there is nothing to install beyond the browser's own install option. Latest production deployment: `https://743db3ef.ai-task-router.pages.dev` from source `0c81132`, deployed 2026-07-26T21:40:00-06:00 on owner authorization, carrying the R-009 catalog review. **Production is one validated stage-repair chunk behind `main`; that repair is not deployed without separate release authorization.** The current local/`main` repair corrects the contradictory GPT-5.5 Medium guidance and the stretched Stage 3 project-plan layout without adding execution behavior. The latest production deploy carried teaching-audit finding 3 — the Best Options impact panel rebalance — plus the fix that makes every comparison multiple divide the dollar figures as displayed rather than the raw estimates behind them; the 11:41:28-06:00 deploy before it carried findings 1 and 2, the pre-task worked examples on Start Here and the cost, energy, and choice-pattern totals on Past Choices; the 11:10:28-06:00 deploy carried the first three teaching-audit fixes (readable energy figures, the followed-route lean/balanced/premium split, comparison multiples); and the 10:21:53-06:00 deploy carried R6, R7, R8, R9, the bundle budget gate, the desktop-track removal, and the leanness pass, so the Help tab shows a real screen and route costs read in real dollars and cents. The Cloudflare deploy blocker is resolved — the token was never the problem, only the network location it was used from, and the recovery path in `docs/2026-07-09-cloudflare-deploy-turnover.md` has now worked unchanged from the home network six times. Use that note as the deploy runbook for the next release, and read its Verification Traps section before recording what is live: both traps have now fired on real deploys. A plain fetch of the canonical URL can report a stale build that is not stale, and a bundle string check can run against a 962-character SPA fallback and report a clean R-010 pass it never earned. Note the newest variant: appending a cache-busting query string to an *asset* path triggers the fallback, so cache-bust the HTML but request the asset path clean, and always assert the fetched length against the build output. Suggested-stage guidance currently lives in `src/domain/routing/stageGuidance.ts` and `src/ui/screens/StageGuidancePanel.tsx`. The latest routing-feedback fix makes chart points expose hover/focus values, makes routing detail show concrete decision/reason/check/upgrade fields, makes complex build-planning tasks split the build stage into concrete build items instead of collapsing into prompting only, and keeps ordinary planning wording like "build an itinerary" out of app-build routing unless the task asks for an app, tool, code, automation, workflow, or build slice. The PWA service worker uses the `ai-task-router-pwa-v2` cache and network-first same-origin asset fetching so returning browsers do not prefer stale app bundles after a release.
