...
The browser/PWA app is live at `https://ai-task-router.pages.dev/`, the Old Skool AI public hub is live at `https://oldskoolai.com/ai-task-router/`, The desktop track was abandoned on 2026-07-26 - the Tauri shell, local discovery, packaging scripts, and the "Desktop app - Coming Soon!" card are gone, and [ADR-0002](docs/decisions/adr-0002-abandon-desktop-track.md) records why. This is a web app; there is nothing to install beyond the browser's own install option. Latest production deployment: `https://eb485f5c.ai-task-router.pages.dev` from source `e3671b2`, deployed 2026-07-27 on owner authorization. It includes the Stage 2 and Stage 3 repair plus the followed-choice clarification: the panel now names Accepted or Edited choices, explains why legacy Deferred or Rejected saves do not count, and links directly to Past Choices for review. **The production application build is current; the only post-deploy `main` changes are the hosted-test readiness fix and release records.** Canonical production serves the exact 661,862-byte local asset with the new panel markers present, the old exact header absent, and hosted E2E 7/7. Use `docs/2026-07-09-cloudflare-deploy-turnover.md` as the deploy runbook and read its Verification Traps section before recording what is live. A plain fetch of the canonical URL can report a stale build that is not stale, and a bundle string check can run against a 962-character SPA fallback and report a clean R-010 pass it never earned. Cache-bust the HTML, request the asset path clean, and always assert the fetched length against the build output. Suggested-stage guidance currently lives in `src/domain/routing/stageGuidance.ts` and `src/ui/screens/StageGuidancePanel.tsx`. The PWA service worker uses the `ai-task-router-pwa-v2` cache and network-first same-origin asset fetching so returning browsers do not prefer stale app bundles after a release.
