...
Current coder queue: [docs/2026-07-25-audit-remediation-plan.md](docs/2026-07-25-audit-remediation-plan.md), chunks R0 through R12 from the 2026-07-25 functional audit. Each chunk names its own context load. All thirteen chunks, R0 through R12, are complete as of 2026-07-25T16:58:01-06:00, and both owner decisions are recorded in that file. There is no next coder chunk in that queue, and the production deploy it was waiting on landed 2026-07-26T10:21:53-06:00. A separate teaching audit on 2026-07-26 then asked whether the app teaches *why* right-sizing matters and found six gaps; all six are fixed and live as of `e58f81b`. The 2026-07-27 Stage 2 reasoning and Stage 3 project-plan repair is complete and live. The same-day followed-choice investigation found that legacy saved plans remained Deferred while the counter only counted Accepted or Edited outcomes; the explanation and direct review path are fixed and live. The later direct-work patch removed an unconditional prompt relay but did not make stage and mode selection task-dependent enough; source `1b5ea8f` replaces that fixed sequencing with task-reasoning, stage-utility, and mode-adequacy decisions and is live. The remaining open items are in `CARRY_FORWARD.md`. Note that R5 turned on `noUnusedLocals` and `noUnusedParameters`, so unused code now fails `npm run build`.
...
The browser/PWA app is live at `https://ai-task-router.pages.dev/`, the Old Skool AI public hub is live at `https://oldskoolai.com/ai-task-router/`, The desktop track was abandoned on 2026-07-26 - the Tauri shell, local discovery, packaging scripts, and the "Desktop app - Coming Soon!" card are gone, and [ADR-0002](docs/decisions/adr-0002-abandon-desktop-track.md) records why. This is a web app; there is nothing to install beyond the browser's own install option. Latest production deployment: `https://0e76221f.ai-task-router.pages.dev` from source `1b5ea8f`, deployed 2026-07-27 on owner authorization. It includes the task-reasoning engine repair: the router evaluates reasoning demand and whether each optional stage adds value, then checks the selected mode against that task and role. With the same saved tools, a structured multi-part plan and a simple rewrite now receive materially different modes and work paths. **The production application build is current; the only post-deploy `main` changes are release records.** Canonical production serves the exact 678,109-byte local asset with the new reasoning and stage-utility markers present, both stale script/default markers absent, and hosted E2E 8/8. Use `docs/2026-07-09-cloudflare-deploy-turnover.md` as the deploy runbook and read its Verification Traps section before recording what is live. A plain fetch of the canonical URL can report a stale build that is not stale, and a bundle string check can run against a 962-character SPA fallback and report a clean R-010 pass it never earned. Cache-bust the HTML, request the asset path clean, and always assert the fetched length against the build output. Suggested-stage guidance currently lives in `src/domain/routing/stageGuidance.ts` and `src/ui/screens/StageGuidancePanel.tsx`. The PWA service worker uses the `ai-task-router-pwa-v2` cache and network-first same-origin asset fetching so returning browsers do not prefer stale app bundles after a release.
